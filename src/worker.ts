	...
	try {
		...
		if (crawler) {
			try {
				...
				logger.warn(
					{ jobId, error: cleanupError instanceof Error ? cleanupError.message : cleanupError },
		...
		const backoffWithJitter = BACKOFF_DELAY_MS * Math.pow(2, job.attempts - 1) * (0.5 + Math.random() * 0.5);
